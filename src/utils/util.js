 ...
 * @param {String} filename 文件名称
 */
export const downloadWord = (fileArrayBuffer , filename) => {
...
export const formatAfterConvertImageName = ({fileName , fileType}) => {
    let index = fileName.lastIndexOf('.')
    if(index===-1)return (fileName+fileType)
    let name = fileName.substring(0,index)
    let type = fileName.substring(index,fileName.length)
